...
## Generated file: src/height-gen.js

`src/height-gen.js` is generated by `scripts/gen-height.mjs` from `src/shaders/terrain.glsl` --
never hand-edit it, re-run `node scripts/gen-height.mjs` after editing the terrain HEIGHT functions.
The GLSL is the single source of truth. `g` in the generated file is the GLSL runtime shim
(`glsl-rt.js`); `U` is uniform values; `hpfSample` is supplied by the caller (the pure-JS
anchor-field) via the U/ctx closure. GPU-texture-only calls reachable from a transpiled height fn
(`sculptOverrideAt` etc, see `STUB_FNS` in `gen-height.mjs`) are emitted as fixed-value stubs, not
transpiled. The generator emits no comment header into the generated file (comment ban); this
section is the durable record of that rationale.
