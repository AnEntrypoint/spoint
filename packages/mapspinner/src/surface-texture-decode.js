...
export async function decodeSurfaceTextureSet(baseUrl) {
  ...
  const albAll = new Uint8Array(SZ * SZ * 4 * SPLAT_LAYER_MATS.length);
  const nrmAll = new Uint8Array(SZ * SZ * 4 * SPLAT_LAYER_MATS.length);
  const _tex = (n) => new URL(n, base).href;
  for (let m = 0; m < SPLAT_LAYER_MATS.length; m++) {
    const [c, d, nj] = await Promise.all([px(_tex(SPLAT_LAYER_MATS[m] + '-color.jpg')), px(_tex(SPLAT_LAYER_MATS[m] + '-displacement.jpg')), px(_tex(NRM_JPGS[m])).catch(() => null)]);
      ...
      for (let y = 0; y < SZ; y++) {
        ...
        for (let x = 0; x < SZ; x++) {
          ...
          for (let ch = 0; ch < 3; ch++) {
            ...
            const s = Math.min(DESHADE_GAIN_MAX, Math.max(DESHADE_GAIN_MIN, Math.pow(gMean[ch] / Math.max(blur, 1e-4), DESHADE_GAIN_EXPONENT)));
            const v = lin(c[i + ch]) * s;
            c[i + ch] = delin8(Math.max(0, gMean[ch] + (v - gMean[ch]) * FINE_CONTRAST_STRETCH));
          }
        }
      }
    }
  ...
  let rockMean;
  { let r = 0, g = 0, b = 0; const rockLayerBase = ROCK_LAYER_INDEX * SZ * SZ * 4, n = SZ * SZ;
    for (let i = 0; i < n; i++) { r += albAll[rockLayerBase + i * 4]; g += albAll[rockLayerBase + i * 4 + 1]; b += albAll[rockLayerBase + i * 4 + 2]; }
  ...
  return { albAll, nrmAll, meanL, rockMean, matCount: SPLAT_LAYER_MATS.length, sz: SZ };
}
