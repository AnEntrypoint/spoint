...
- Server/security: auth matrix with `EDITOR_TOKEN` unset (`server-http-auth-matrix`); SandboxEvaluator proxy tier is not a boundary (`sandbox-evaluator-proxy-tier-not-a-boundary`); static path containment + COOP/COEP (`statichandler-path-containment`, `statichandler-coop-coep-require-corp`).
