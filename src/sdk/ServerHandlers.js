...
export function createConnectionHandlers(ctx) {
  ...
  connections.on('message', (clientId, msg) => {
    ...
    if (msg.type === MSG.APP_EVENT) {
      ...
      for (const [entityId] of appRuntime.apps) appRuntime.fireMessage(entityId, eventData)
      return
    }
    if (msg.type === MSG.RECONNECT) {
      ...
      const clientModules = appLoader.getClientModules()
      for (const [appName, code] of Object.entries(clientModules)) connections.send(newId, MSG.APP_MODULE, { app: appName, code })
