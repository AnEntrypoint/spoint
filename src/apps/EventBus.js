export class EventBus {
  ...
  on(channel, handler) {
    ...
    if (channel.endsWith('*')) this._rebuildWildcardTrie()
  ...
  emit(channel, data, meta = {}) {
    ...
    let node = this._wildcardTrie
    for (let i = 0; i < channel.length && node; i++) {
      const c = channel[i]
      if (node.__handlers) for (const fn of node.__handlers) { try { fn(event) } catch (e) { console.error(`[EventBus] *${channel.slice(0,i)}*:`, e.message) } }
      node = node.get(c)
    }
    if (node?.__handlers) for (const fn of node.__handlers) { try { fn(event) } catch (e) { console.error(`[EventBus]:`, e.message) } }
    return event
  }
