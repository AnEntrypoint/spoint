...
class FacialAnimationPlayer {
  constructor(vrm, opts={}) {
    ...
    this._exprValuesBuffer=new Map()
  ...
  applyFrame(bs) {
    if (!this.expressionManager) return
    const vals=this._exprValuesBuffer; vals.clear(); const has=n=>this.availableExpressions.has(n), set=(n,v)=>{if(has(n)&&v>0.001)vals.set(n,clamp(v))}
