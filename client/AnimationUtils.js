...
export function remapMixamoClip(clip, targetBoneMap, validBones) {
  ...
  for (const track of clip.tracks) {
    ...
    const rawName = (dot >= 0 ? track.name.slice(0, dot) : track.name).replace(/^mixamorig:?/, '')
    const prop = dot >= 0 ? track.name.slice(dot) : ''
    const semantic = _MIXAMO_NAME_TO_SEMANTIC[rawName]
    const mapped = (semantic && targetBoneMap[semantic]) || rawName
