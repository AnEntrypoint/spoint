...
### `apps/placed-model` — drop in a map/interior model with a real collider (static meshes collide by default)
A dropped-in map or interior model needs no dedicated app: use the built-in `placed-model` app with
`config.collider` (box/sphere/capsule/convex/trimesh/none, trimesh-with-box-fallback baked from the GLB)
and `custom._interior: true` for DoubleSide interior-face rendering + always-relevant snapshot inclusion
(the map/arena is always visible to every player in it, not distance-culled like a normal prop).
...
export default {
  ...
  // { urls: string|string[], username?: string, credential?: string }. STUN-only entries omit
  // username/credential. Omitted entirely, or [], falls back to wireweave's own default (a public
  // STUN server + a shared-credential demo TURN relay -- fine for local testing, not for a real
  // hosted deployment since every NAT-restricted peer then depends on that shared free-tier relay).
  iceServers: [
...
A **live overlay** of the entity's editor-facing state: `ctx.config` reads the
world-def `config:{...}` PLUS every non-`_`-prefixed key of `ctx.entity.custom`
(so an `editorProps` field a maker edits in the inspector shows up here). Set in
world: `{ id:'x', app:'y', config:{ radius:5 } }` → `ctx.config.radius`. When the
editor edits a prop live, `ctx.config` updates and any `ctx.onConfigChange(cb)`
listeners fire — no re-`setup`:
```js
setup(ctx) {
  ctx.onConfigChange(cfg => { /* re-read cfg.radius etc. — editor prop changed live */ })
}
```
...
ctx.physics.getVelocity() / getAngularVelocity() / getRotation()   // read this body's live motion
ctx.physics.isAtRest(eps=0.05)                // settled? (linear+angular speed below eps) — tower-topple lose check
ctx.physics.tiltFromUpright()                 // radians from vertical (toppled piece ~PI/2)
ctx.physics.setFriction(f) / setRestitution(b)// per-body friction / bounciness at runtime
```
Cross-entity + off-centre physics (via `ctx.world` / `ctx.physics`):
```js
ctx.world.applyImpulse(entityId, [ix,iy,iz], worldPoint?)  // worldPoint => torque (off-centre hit)
ctx.world.setVelocity(entityId, [vx,vy,vz])
ctx.world.setPosition(entityId, [x,y,z], rotation?)        // kinematic move (entity + collider) — moving platforms
ctx.world.setGravityFactor(entityId, factor)               // 1 = normal, 0 = float, <0 = anti-gravity
ctx.world.weld(entityA, entityB, opts?)                    // fixed constraint
ctx.world.joint(entityA, entityB, { type:'point'|'distance'|'hinge', ... })
ctx.world.removeConstraint(constraintId)
```
...
ctx.players.setName(playerId, name)            // server-authoritative display name; read via getById(id).name
ctx.players.setAppearance(playerId, { tint, nameTag })  // per-player mesh tint (0xRRGGBB|null) + nameplate
ctx.players.setModel(playerId, url)            // swap one player's whole avatar VRM/GLB (skins, team models)
ctx.players.setMovementOverride(playerId, { maxSpeed, jumpImpulse, ... })  // null = clear
ctx.players.onPlayerContact(radius, cb)        // cb(idA,idB) per unordered in-radius pair/tick — tag/sumo/dodgeball
ctx.players.nearestOtherPlayer(playerId, radius)  // nearest player excluding self, or null
ctx.players.setLifecycle(playerId, 'alive'|'frozen'|'spectator', { spectateTarget })  // elimination/freeze-tag
ctx.players.playAnimation(playerId, clip, { loop, fade })  // force a clip on that player's avatar
ctx.players.attachEntity(playerId, entityId, [ox,oy,oz])   // entity follows the player (flag carry, held tool)
ctx.players.detachEntity(entityId)
```
Mutate `player.state.health` / `player.state.velocity` directly — propagates in next snapshot.
Client-side follow-cam: `engine.spectate(targetPlayerId)` (pass null to return to own view).
...
### ctx.raycast
```js
const hit = ctx.raycast([x,y,z], [dx,dy,dz], maxDist, excludeBodyId?)
// { hit:bool, distance:number, position:[x,y,z]|null, normal:[x,y,z]|null,
//   bodyId:number|null, entityId:string|null }
// entityId attributes the hit to a spawned entity (null = terrain/unowned body) —
// this is what a shoot/click/grab-a-target game reads to know WHAT was hit.
```

### ctx.define* — reusable game-mechanic factories

Prefer these over hand-rolling common mechanics in `update()` (all take a spec,
return a stateful handle you drive from `update`/`onInteract`):
```js
ctx.defineGameFSM(spec)       // declarative match phases (any custom state machine) — xstate5
ctx.defineGameMode(spec)      // lobby/warmup/rounds/end match-loop skeleton over defineGameFSM+defineTeams
                              // ({teams?,minPlayers,countdownMs,roundMs,intermissionMs,roundsPerMatch,winCondition,scoring})
                              // -> .teams/.round/.addScore/.getScore/.getScores + defineGameFSM's own runtime
ctx.defineHealth(spec)        // HP / damage / death / respawn
ctx.defineSteering(spec)      // seek / arrive / separation / ground-clamp AI movement
ctx.defineCheckpoint(spec)    // per-player checkpoint + fall-respawn
ctx.defineBuffStack(spec)     // stackable timed buffs/debuffs
ctx.defineShrinkingZone(spec) // battle-royale storm circle
ctx.definePickup(spec)        // nearest-player-in-radius collect trigger ({radius,onCollect,oneShot,cooldown})
ctx.defineDestructible(spec)  // health + impact-damage + debris + respawn for a breakable object
ctx.defineTeams(spec)         // server-authoritative team membership + shared scoreboard (CTF/TDM/KotH/domination)
                              // ({teams,autoBalance,scoreLimit,onAssign,onScore,onWin}) -> assignPlayer/addScore/leader/getScores
ctx.defineWeapon(spec)        // hitscan/ammo/reload/killfeed combat ({damage,range,magazine,reloadMs,fireRateMs,onFire,onHit,onKill})
                              // -> fire(shooterId,origin,dir,opts)/reload()/canFire()/ammo
ctx.definePlayerInventory(spec)// per-player items + currency/buy-phase ({startItems,startCurrency,catalog,onChange,onBuy})
                              // -> add/remove/count/has/currency/earn/spend/buy/push(pid)
ctx.definePath(points)        // ordered-waypoint path -> length/pointAt(d)/progressAt(pos)/directionAt(d) (race/patrol/payload)
```
...
```js
ctx.onPlayerProximity(radius, cb)  // cb(ctx, playerId) once/tick per player within radius of this entity
ctx.onConfigChange(cb)             // cb(config) when the editor edits an editorProp live (no re-setup)
```

### ctx.canSee — line-of-sight
```js
ctx.canSee(fromPos, toPos, { excludeBodyId?, targetEntityId?, maxDistance?, tolerance? })
// true if toPos is visible from fromPos with nothing solid between them (raycast-based).
// Stealth vision cones, hide-and-seek detection, red-light-green-light "is the mover in view".
```
...
engine.pickGround(clientX, clientY)          // world point under a screen coord -> [x,y,z]|null (RTS/tower/golf)
engine.followEntity(entityId, { distance, height })  // orbit cam around a non-player entity (ball-cam); null = back to player
engine.spectate(targetPlayerId)              // follow-cam another player; null = own view
```
