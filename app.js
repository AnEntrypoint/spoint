...
import { PhysicsNetworkClient, InputHandler, MSG } from '/spoint/src/index.client.js'
...
async function checkAllLoaded() { if (loadingScreenHidden||!assetsLoaded||!environmentLoaded||!firstSnapshotReceived||!modelsPrefetched||firstSnapshotEntityPending.size>0) return; loadingScreenHidden=true; loadingMgr.setLabel('Starting game...'); try { if (!_isSingleplayer || el.entityMeshes.size < 10) { await warmupShaders(renderer,scene,camera,el.entityMeshes,pm.playerMeshes,loadingMgr) } else { console.log('[shader] skipping warmup in singleplayer with',el.entityMeshes.size,'entities'); loadingMgr.setLabel('Starting game...') } } catch (_) {}; el.onMeshReady=m=>{ try { renderer.compileAsync(scene,camera).catch(()=>{}) } catch(_){} }; loadingScreen.hide() }
...
function updatePlayerPositions(players, lid, frameDt) {
  for (const p of players) {
    ...
    if (p.id===lid) { const lc=client.getLocalState(); const lp=lc?.position||p.position; vx=lc?.velocity?.[0]||0;vy=lc?.velocity?.[1]||0;vz=lc?.velocity?.[2]||0; tx=lp[0]; ty=lp[1]-fo; tz=lp[2] }
