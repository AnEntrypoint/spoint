...
| `Vegetation.js` | `onBeforeCompile` (wind displacement, shared with tree trunk/branch) | Tree/vegetation wind animation | **Needs TSL rewrite, medium risk.** This row previously cited an AGENTS.md slug `tree-flicker-root-cause-2026-07-11` as grounds for a HIGH RISK label; that slug does not exist in recall, `.gm/mutables.yml`, or git history (self-citation error, corrected 2026-09-22). The real, verified prior tree-flicker incident (`mem-a6f1d7bd7be3412c-3784`) was caused by insufficient `polygonOffsetFactor`/`polygonOffsetUnits` (z-fighting against terrain), fixed in `VegetationBuild.js`'s `buildSpecies` -- unrelated to wind-displacement vertex math. A TSL port of the wind shader does not touch polygon-offset settings and does not risk that regression; treat this the same as the other medium-risk full-vertex-math ports (`WetnessTint`, `SSR`, `PlayerVAT`, `PlayerLOD`). |
...
| **TSL-portable, medium risk** (per-fragment math correctness matters, or touches a historically fragile subsystem) | WetnessTint, SSR, PlayerVAT, PlayerLOD, Vegetation.js wind shader (reclassified from HIGH risk 2026-09-22 -- see corrected row above) |
| **TSL-portable, HIGH risk** (file's own header documents real prior GLSL-string-surgery bugs it already hit and fixed) | CascadeShadowSelect.js |
