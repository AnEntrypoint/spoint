...
- **`AppLoader.loadFromString` (browser Worker path)**: uses `URL.createObjectURL + import()` instead of `new Function` — handles full ES module syntax (`export default`, top-level `await`). Apps loaded via this path **cannot use relative `import` statements** (blob URL has no base path for resolution); all inter-app dependencies must be inlined or passed via `engineCtx`. The `"app"` field on entities in `singleplayer-world.json` must be set for `BrowserServer.connect()` to fetch and send the app source to the Worker — missing `"app"` means `setup()` never runs and no collider is created.
- **`singleplayer-world.json` env-map entity**: must have `"app": "environment"` — without it, the environment app is not fetched, `setup()` never runs, and the trimesh collider is not created (players fall through the floor in browser singleplayer)
