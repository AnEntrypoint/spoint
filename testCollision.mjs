...
async function testCharacterCollision() {
    ...
    // Create floor at y=0
    const floorId = world.addStaticBox([10, 5, 10], [0, -5, 0]);
    ...
    for (let i = 0; i < steps; i++) {
        ...
        if (i % 10 === 0) {
            ...
            console.log(`Step ${i}: pos=${pos[1].toFixed(2)}, vel=${newVel[1].toFixed(2)}, onGround=${onGround}`);
        }
    }
    ...
    // Verify character is on ground and stopped falling
    const expectedY = 5.0 + 0.4 + 0.9; // floor height 5 + capsule radius 0.4 + capsule half height 0.9
